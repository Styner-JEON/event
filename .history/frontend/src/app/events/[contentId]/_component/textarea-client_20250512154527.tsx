...
async function sendRequest(url, { arg }: { arg: { content: string }}) {
  return fetch(url, {
    method: 'POST',
...
export default function TextareaClient({ contentId }: { contentId: string}) {    
  const endpoint = 
  const { trigger, data, error, isMutating } = useSWRMutation(`${process.env.NEXT_PUBLIC_CONTENT_BASE_URL}/events/${process.env.NEXT_PUBLIC_CONTENT_API_VERSION}/${contentId}/comments`, sendRequest)
