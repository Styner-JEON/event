...
export async function login(state: FormState, formData: FormData) {  
  ...
  const tokenExpirationTime = parseInt(process.env.TOKEN_EXPIRATION_TIME || '604800000');
  const accessTokenExpiresAt = new Date(Date.now() + ACCESS_TOKEN_EXPIRATION_TIME);
  await createSession('accessToken', accessToken, accessTokenExpiresAt);
  
  const refreshTokenExpiresAt = new Date(Date.now() + tokenExpirationTime);
