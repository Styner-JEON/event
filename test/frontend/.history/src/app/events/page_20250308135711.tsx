    ...
    <main>      
      ...
      {content.map((event: EventResponse) => (
        <div key={event.contentId}>
          <h2>{event.title}</h2>
          {event.firstImage && <Image src={event.firstImage} alt={event.title} width={300} height={300} />}
          {event.firstImage2 && <Image src={event.firstImage2} alt={event.title} width={300} height={300} />}
        </div>
      ))}
    </main>
  );
